...
def get_bert_classification(request, user_input):
    ...
    return most_likely_class, all_classes_n_score


def fetch_explainations_rag(request, user_input, top_k):

    embedding = request.app.state.EMBEDDINGS_MODEL.encode(user_input)
    search_result = request.app.state.QDRANT_CLIENTT.search(
        collection_name="HateXplain_gpu_stella_0", query_vector=embedding, limit=top_k
    )
...
async def generate_chat_completion(
    ...
    # new_message = ChatMessage(role="system", content=system_prompt_classification)
    # classified_payload['messages'].append(new_message.dict())
    ...
    #     request.app.state.config.OLLAMA_API_CONFIGS.get(url, {}),  # Legacy support
    # )
    # classified_response = await send_post_request(
    #     url=f"{url}/api/chat",
    #     payload=json.dumps(classified_payload),
    #     stream=False,
    #     key=get_api_key(url_idx, url, request.app.state.config.OLLAMA_API_CONFIGS),
    ...
    # new_message = ChatMessage(role="system", content="WRITE A STORY ABOUT NASA")
    #

    # send_post_request

    # Append the new message to the messages list
    # payload['messages'].append(new_message.dict())
    ...
    #     # First, log the stream to verify content
    #     async for chunk in stream:
    #         print(f"Received chunk: {chunk}")
    #         if chunk.strip():  # If the chunk is non-empty
    #             yield chunk
    #         else:
    #             print("Empty chunk received")

    # Prepare for response streaming
    async def stream_wrapper(stream, all_classes_n_score, model_name):
        # Yield the all_classes_n_score as the first chunk
        # yield json.dumps()all_classes_n_score
        # yield all_classes_n_score.encode("utf-8")
        ...
        chunk_data = {
            ...
            "message": {
                "role": "assistant",
                "content": all_classes_n_score  # Insert all_classes_n_score into content
            },
            "done": False
        }

        # Convert the dictionary to JSON string and then encode to bytes
        chunk_data_bytes = json.dumps(chunk_data).encode("utf-8")

        # Yield the all_classes_n_score as the first chunk
        yield chunk_data_bytes

        # Then, stream the rest of the response
        async for chunk in stream:
            # print(f"Received chunk: {chunk}")
            # print(all_classes_n_score)
            # print(type(chunk))
            if chunk.strip():  # If the chunk is non-empty
                yield chunk
            else:
                print("Empty chunk received")
    # async def stream_wrapper(stream, all_classes_n_score):
    #     # First, log all_classes_n_score to check its content before encoding
    #     print(f"All classes and scores: {all_classes_n_score}")
    #
    #     # Yield the all_classes_n_score as the first chunk (convert to bytes)
    #     # Ensure it is valid JSON if needed
    #     if not isinstance(all_classes_n_score, bytes):
    #         all_classes_n_score = all_classes_n_score.encode("utf-8")
    #
    #     yield all_classes_n_score  # First chunk
    #
    #     # Then, stream the rest of the response
    #     async for chunk in stream:
    #         print(f"Received chunk: {chunk}")
    #
    #         # Decode and check the chunk content
    #         try:
    #             # Try decoding the chunk to see if it's valid JSON
    #             chunk_data = chunk.decode("utf-8")
    #             print(f"Decoded chunk: {chunk_data}")
    #
    #             # If the chunk is valid, yield it
    #             yield chunk
    #         except Exception as e:
    ...
    # Return the response as a streaming response with rag_result_str
    return StreamingResponse(
        stream_wrapper(response.body_iterator, all_classes_n_score, payload["model"]),
